...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-text">View Contact</h1>
        ...
        <div class="bg-gradient-to-tl from-[#23272b] via-[#e2e2e2]/10 to-[#1e2024] rounded-lg shadow-lg p-6">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div>
                    ...
                    <h3 class="text-lg font-semibold text-text mb-3">Email</h3>
                    <p class="text-muted mb-6">{{ $contact->email }}</p>

                    @if($contact->phone)
                        <h3 class="text-lg font-semibold text-text mb-3">Phone</h3>
                        <p class="text-muted mb-6">{{ $contact->phone }}</p>
                    @endif

                    @if($contact->address)
                        <h3 class="text-lg font-semibold text-text mb-3">Address</h3>
                        <p class="text-muted mb-6 whitespace-pre-wrap">{{ $contact->address }}</p>
                    @endif

                    @if($contact->social_links)
                        <h3 class="text-lg font-semibold text-text mb-3">Social Links</h3>

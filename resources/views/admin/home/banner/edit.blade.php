...
<div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
    <div class="">
            ...
            <div class="flex justify-between items-start">
                    ...
                    <div class="flex flex-wrap items-center gap-4">
                        @if($banner->exists)
                            ...
                            <div class="flex items-center bg-blue-50 px-5 py-3 rounded-xl border border-blue-200">
                                ...
                                <span class="text-gray-700 font-medium">Created: {{ $banner->created_at->format('M d, Y') }}</span>
                            </div>
                        @else
                            <div class="flex items-center bg-blue-50 px-5 py-3 rounded-xl border border-blue-200">
                                <svg class="w-5 h-5 mr-3 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex space-x-4">
                    <div class="text-center p-4 bg-white rounded-2xl shadow-lg border border-gray-200 min-w-[80px]">
                        <div class="text-3xl font-bold {{ $banner->background_image ? 'text-green-600' : 'text-red-500' }} mb-1">
                            {{ $banner->background_image ? '✓' : '✗' }}
                        </div>
                        <div class="text-sm text-gray-600 font-medium">Background</div>
                    </div>
                    
                    <div class="text-center p-4 bg-white rounded-2xl shadow-lg border border-gray-200 min-w-[80px]">
                        <div class="text-3xl font-bold {{ $banner->person_image ? 'text-green-600' : 'text-red-500' }} mb-1">
                            {{ $banner->person_image ? '✓' : '✗' }}
                        </div>
                        <div class="text-sm text-gray-600 font-medium">Profile</div>
                    </div>
                    
                    <div class="text-center p-4 bg-white rounded-2xl shadow-lg border border-gray-200 min-w-[80px]">
                        <div class="text-3xl font-bold {{ $banner->cv_file ? 'text-green-600' : 'text-red-500' }} mb-1">
                            {{ $banner->cv_file ? '✓' : '✗' }}
                        </div>
                        <div class="text-sm text-gray-600 font-medium">CV</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        >
            <!-- Banner Content -->
            <div class="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-3xl p-8 mb-8 border border-indigo-200 shadow-lg">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-lg border border-gray-200">
                        ...
                        <div class="space-y-3">
                            <div class="text-2xl font-bold text-gray-900">{{ $banner->title_line1 ?: 'Your Title Line 1' }}</div>
                            <div class="text-xl font-semibold text-blue-600">{{ $banner->title_line2 ?: 'Your Title Line 2' }}</div>
                            <div class="text-gray-600 text-sm">{{ $banner->subtitle ?: 'Your subtitle will appear here...' }}</div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-3xl p-8 mb-8 border border-blue-200 shadow-lg">
                <h3 class="text-2xl font-bold text-gray-900 mb-6 flex items-center">
                    <div class="w-10 h-10 bg-gradient-to-r from-blue-500 to-cyan-600 rounded-2xl flex items-center justify-center mr-4 shadow-lg">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        </svg>
                    </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div class="bg-white rounded-2xl p-6 shadow-lg border border-gray-200">
                        ...
                        @if($banner->background_image)
                            <div class="mb-6 p-4 bg-gradient-to-r from-gray-50 to-gray-100 border border-gray-200 rounded-xl">
                                ...
                                <div class="relative group">
                                    ...
                                    @endphp
                                    @if($isVideo)
                                        <video class="w-full h-48 object-cover rounded-xl border border-gray-200 shadow-lg group-hover:shadow-xl transition-all duration-300" autoplay muted loop playsinline>
                                            <source src="{{ asset('storage/' . $banner->background_image) }}" type="video/{{ $ext == 'webm' ? 'webm' : 'mp4' }}">
                                        </video>
                                    @else
                        ...
                        <div class="space-y-4">
                            <input id="homeBackgroundMediaInput" type="file" name="background_image" accept="image/*,video/mp4,video/webm" 
                            ...
                            <div id="homeMediaPreviewWrapper" class="hidden">
                                ...
                                <div class="relative w-full rounded-xl overflow-hidden border border-gray-200">
                                    <div id="homeLoadingOverlay" class="absolute inset-0 bg-black/40 flex items-center justify-center">
                                    ...
                                    <div class="bg-gray-100">
                                        <video id="homeVideoPreview" class="w-full h-auto hidden" autoplay muted loop playsinline></video>
                                        <img id="homeImagePreview" class="w-full h-auto hidden" alt="Selected preview" />
                                    </div>
                                </div>
                    ...
                    @if($banner->cv_file)
                        <div class="mb-6 p-4 bg-gradient-to-r from-gray-50 to-gray-100 border border-gray-200 rounded-xl">
                            ...
                            <div class="flex items-center space-x-4 p-4 bg-white rounded-xl border border-gray-200 shadow-sm">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm font-semibold text-gray-900">{{ basename($banner->cv_file) }}</p>
                                    <a href="{{ asset('storage/' . $banner->cv_file) }}" target="_blank" 
                                       class="text-xs text-blue-600 hover:text-blue-800 underline font-medium">View File</a>
                                </div>
                            </div>
                        </div>
                    @endif
      ...
      input.addEventListener('change', function(e){
        ...
        if (file.type && file.type.startsWith('video/')) {
          if (video) {
            ...
            const onLoaded = function(){ hideOverlay(); video.removeEventListener('loadeddata', onLoaded); };
            const onErr = function(){ hideOverlay(); video.removeEventListener('error', onErr); };

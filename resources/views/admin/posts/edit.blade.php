...
<div class="container mx-auto px-6 py-8">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-8">
            <h1 class="text-3xl font-bold text-gray-900">Edit Post: {{ $post->title }}</h1>
            ...
            <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <select name="type" id="type" required 
                        ...
                        <option value="project" {{ $post->type === 'project' ? 'selected' : '' }}>Project</option>
                        <option value="blog" {{ $post->type === 'blog' ? 'selected' : '' }}>Blog Post</option>
                        <option value="service" {{ $post->type === 'service' ? 'selected' : '' }}>Service</option>
                    </select>
                    @error('type')
                ...
                <div class="mb-6">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <p class="text-sm text-gray-600">Current image: {{ basename($post->image) }}</p>
                ...
                <div id="project-fields" class="mb-6 {{ $post->type === 'project' ? '' : 'hidden' }}">
                    <div class="mb-4">
                        <label for="github_link" class="block text-sm font-medium text-gray-700 mb-2">GitHub Link</label>
                        <input type="url" name="github_link" id="github_link" value="{{ old('github_link', $post->github_link) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                ...
                <div id="service-fields" class="mb-6 {{ $post->type === 'service' ? '' : 'hidden' }}">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="icon" class="block text-sm font-medium text-gray-700 mb-2">Icon (CSS class or emoji)</label>
                            <input type="text" name="icon" id="icon" value="{{ old('icon', $post->icon) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        ...
                        <div>
                            ...
                            <input type="number" name="order" id="order" value="{{ old('order', $post->order ?? 0) }}" min="0"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @error('order')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
